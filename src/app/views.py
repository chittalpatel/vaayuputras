...
def wffromdb(request):
    ...
    if custom_model == 'no':
        ...
        model=TrainModel(None,inputwindspeed,None,float(windfarmDetails['Capacity']),False)
    ...
    staticmaps=f"https://open.mapquestapi.com/staticmap/v4/getmap?key={settings.MAPS_API_KEY}&size=600,400&zoom=9&center={coordinates['Longitude']},{coordinates['Latitude']}"
...
def wfmanual(request):
    if request.method=='POST':
        ...
        lat=request.POST['wflat']
        long = request.POST['wflong']
        ...
        # multiplying by 100 and dividing by 48 to get frequency distribution
        wr = wr * 100 / 48
        # convert to a form in which we can give input to deployed model
        inputwindspeed = [[[ws / 25]] for ws in windspeed]
        # create an object to load model
        model = TrainModel("", inputwindspeed, None, float(windfarmDetails['Capacity']), False)
        # predict the wind power using user personal model.
        windpower = model.predict()
        del model
...
def get_prediction(lat,long,cap):
    ...
    # res = requests.get(
    #     f'https://api.climacell.co/v3/weather/forecast/hourly?lat={lat}&lon={long}&start_time=now&fields[]=wind_speed&fields[]=wind_direction&apikey=53T9LhKGrYz5w6dR0Qm2iV6s9bRsv9yu')
    # output = res.json()
    ...
    api = "random_api_key"
    # create an object to load model
    model = TrainModel(api, inputwindspeed, None, float(cap), False)
    # predict the wind power using user personal model.
    windpower = model.predict()
    del model
